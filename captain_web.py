...
def before_request():
    g.config = Config()
    try:
        app.logger.setLevel(g.config.logging_level)
        app.logger.info("Logging level set to {}".format(g.config.logging_level))
    except ValueError:
        app.logger.error("Unable to configured specified logging level, '{}' is an invalid value.".format(g.config.logging_level))
    g.captain_conn = Connection(g.config, aws_host_resolver)
